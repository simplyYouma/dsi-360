...
import type { LienItem } from '@/common/LiensActivite';
...
export const changementsApi = {
  ...
  // Liens utiles du changement — mêmes routes que les projets. Rattachés au sujet, jamais à une
  // tâche : un lien survit à l'étape qui l'a fait naître.
  liens: (id: string): Promise<LienItem[]> => api.get(`${B}/${id}/liens`),
  creerLien: (id: string, libelle: string, url: string): Promise<LienItem> =>
    api.post(`${B}/${id}/liens`, { libelle, url }),
